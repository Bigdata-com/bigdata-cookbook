"""Short smoke test for Liquid Cooling Market Watch (REST migration)."""
...
from src.labeling import DEFAULT_LABELING_MODEL, run_liquid_cooling_prompt
from src.search import search_liquid_cooling

PROVIDER_QUERIES = [
    "Company provides liquid cooling solutions for data centers.",
]
ADOPTER_QUERIES = [
    "Company adopts liquid cooling technology in its data centers.",
]
...
def main() -> int:
    ...
    df_provider = search_liquid_cooling(
    ...
    df_adopter = search_liquid_cooling(
        sentences=ADOPTER_QUERIES,
        start_date=start_date,
        end_date=end_date,
        company_ids=company_ids,
        id_to_name=id_to_name,
    ...
    df_all = pd.concat([df_provider, df_adopter], ignore_index=True)
