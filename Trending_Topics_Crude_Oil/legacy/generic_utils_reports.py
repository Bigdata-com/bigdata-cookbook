...
async def consolidate_trending_topics(batch_results, model, yearmonth, api_key, main_theme):
...
async def create_titles_from_summaries(df, model='gpt-3.5-turbo', api_key=None):
...
async def process_all_trending_topics(unique_reports, start_query, end_query, main_theme, model='gpt-4o-mini-2024-07-18', api_key=None, batchs=5):
    ...
    for period in dates:
        ...
        batch_size = len(reports) // batchs + (1 if len(reports) % batchs != 0 else 0)
        batch_results = []
    ...
    # **Step 4**: Consolidate all unique topics and summaries across all batches and dates
    ...
    # Step 2: Call the function to get the consolidated topics
    consolidated_topics_and_summaries = await consolidate_trending_topics(', '.join([f'topic_{i + 1}: "{topic}"' for i, topic in enumerate(unique_topics_and_summaries)]), model, yearmonth, api_key, main_theme)

    print(consolidated_topics_and_summaries)
    
    final_mapping = {}
    for consolidated_topic, topic_numbers in consolidated_topics_and_summaries.items():
...
def run_process_all_trending_topics_legacy(unique_reports, start_query, end_query, main_theme, model, api_key, batchs = 5):
    return asyncio.run(process_all_trending_topics(unique_reports, start_query, end_query, main_theme, model, api_key, batchs))
...
async def filter_irrelevant_news(df, model, api_key, main_theme, semaphore_size=1000):
    ...
    filtered_df = df[df['row_number'].isin(relevant_rows)].drop(columns=['row_number'])
