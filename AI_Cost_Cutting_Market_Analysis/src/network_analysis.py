...
def provider_adopter_net(df_filtered_adopters,
    ...
    Compatible with AI cost cutting workflow (users = adopters terminology).
    
    Parameters:
    -----------
    df_filtered_adopters : pd.DataFrame
        DataFrame with adopter/user companies (label "A")
    ...
    """
    return provider_adopter_net_implementation(df_filtered_adopters, df_filtered_providers, bigdata_cred, min_occurrences, dash, interactive)


def provider_adopter_net_implementation(df_filtered_adopters,
                      df_filtered_providers, bigdata_cred,
                      min_occurrences=1,
                      dash=False,
                      interactive=True):
    ...
    if interactive:
        ...
        user_nodes = [node for node in G.nodes() if node.endswith('_user')]
        ...
        if user_nodes:
            ...
            nx.draw_networkx_nodes(G, user_pos, nodelist=user_nodes, 
        ...
        labels = {node: node.replace('_user', '').replace('_provider', '') for node in G.nodes()}
        nx.draw_networkx_labels(G, pos, labels, font_size=8, font_weight='bold', ax=ax)
        
        # Set title and clean up axes
        ax.set_title("Provider-User Network", fontsize=16, fontweight='bold', pad=20)
        ax.axis('off')
        
        # Add legend
        from matplotlib.patches import Patch
        legend_elements = [
            Patch(facecolor='royalblue', label='Users'),
