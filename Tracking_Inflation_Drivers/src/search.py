...
def search_by_any(
    ...
    **kwargs,
    ...
    This function is adapted from search_cooling to work with any theme and 
    supports open discovery when companies=None.
    ...
    Args:
        ...
        companies (Optional[List[Company]]): The list of companies to use. 
            If None, all companies found in search results will be processed, 
            with each company treated as a target entity in separate rows.
    ...
    results = run_search(
        ...
        workflow_name="TrackingInflationDrivers",
        **kwargs,
    )
    results, entities = filter_search_results(results)
    
    # Keep ALL entities before filtering for the additional columns
...
def process_screener_search_results(
    ...
    entities: List[ListQueryComponent],
    all_entities: Optional[List[ListQueryComponent]] = None,
    companies: Optional[List[Company]] = None,
    document_type: DocumentType = DocumentType.NEWS,
    duplicate_for_all_entities: bool = False,
) -> DataFrame:
    """
    ...
    Args:
        ...
        entities (List[ListQueryComponent]): A list of entities (companies only).
        all_entities (Optional[List[ListQueryComponent]]): A list of ALL entities including non-COMP types.
            Used to populate additional columns with entity names and types.
        ...
        duplicate_for_all_entities (bool): If True, creates duplicate rows for each
            entity found in chunks, with each entity treated as target in turn.
    ...
    Returns:
        ...
        - Columns:
            ...
            - masked_text: str
            - other_entities_map: List[Tuple[int, str]]
            - topics: List[str] (names of ALL entities in chunk)
            - topics_type: List[str] (types of ALL entities in chunk)
    """
    entity_key_map = {entity.id: entity for entity in entities}
    
    # Create a map for ALL entities (including non-COMP types) if provided
    all_entity_key_map = {}
    if all_entities:
        all_entity_key_map = {entity.id: entity for entity in all_entities}
    ...
    for result in tqdm(results, desc=f"Processing {document_type} results..."):
        for chunk in result.chunks:
            ...
            # Build a list of ALL entities present in the chunk (including non-COMP types)
            all_chunk_entity_names = []
            all_chunk_entity_types = []
            if all_entity_key_map:
                for entity in chunk.entities:
                    if entity.key in all_entity_key_map:
                        entity_obj = all_entity_key_map[entity.key]
                        entity_name = getattr(entity_obj, 'name', None)
                        entity_type = getattr(entity_obj, 'entity_type', None)
                        if entity_name:
                            all_chunk_entity_names.append(entity_name)
                        if entity_type:
                            all_chunk_entity_types.append(entity_type)
            ...
            if document_type in (DocumentType.FILINGS, DocumentType.TRANSCRIPTS):
                ...
                reporting_entities = result.reporting_entities or []
                
                if reporting_entities:
                    # Standard case: transcript has reporting entities
                    for re_key in reporting_entities:
                        ...
                        rows.append(
                            {
                                ...
                                "text": chunk.text,
                                "other_entities": ", ".join(
                                    e["name"] for e in other_entities if e["name"]
                                ),
                                "entities": chunk_entities,
                                "topics": all_chunk_entity_names,
                                "topics_type": all_chunk_entity_types,
                            }
                        )
                else:
                    # Fallback case: transcript has no reporting entities
                    # Treat it like news/other documents for open discovery
                    if duplicate_for_all_entities:
                        # Create a row for each entity in the chunk, treating each as target
                        for chunk_entity in chunk_entities:
                            ...
                            # Exclude the current entity from other entities
                            other_entities = [
                                e for e in chunk_entities if e["key"] != chunk_entity["key"]
                            ]
                ...
                if duplicate_for_all_entities:
                    ...
                    # Original logic: filter by companies if provided
                    for chunk_entity in chunk_entities:
                        entity_key = entity_key_map.get(chunk_entity["key"])
