...
class TopicSummarizerSector:
    def __init__(self, model: str, api_key: str, df_labeled: pd.DataFrame, list_specific_themes: List[str], 
    ...
    def summarize(self, import_from_path: Optional[str] = None, 
        ...
        if import_from_path and os.path.isfile(import_from_path):
            logging.info("Importing summaries from pickle file.")
        ...
        for main_theme in self.list_specific_themes:
            list_topics = get_most_granular_elements(self.themes_tree_dict[main_theme], 'Label')

            for topic in list_topics:
                df_to_summarize = df_topics_themes.loc[
                    (df_topics_themes.theme == main_theme) &
                ...
                # Summarization logic
                text_to_summarize = self.generate_summary(list_text_to_summarize, self.model, n_reports, self.api_key, main_theme, topic)

                summary = topic_summary(text_to_summarize, self.model, n_reports, self.api_key, main_theme, topic)
        ...
        if list_df_by_theme:
            ...
            if export_to_path:
                ...
                logging.info("Exported summaries to pickle file.")

            return df_by_theme

        logging.warning("No summaries were produced; returning None.")
        return None
...
class TopicSummarizerCompany:
    ...
    """
    def __init__(self, model: str, api_key: str, max_tokens: int = 100000, verbose: bool = False) -> None:
        """
    ...
    async def process_entity_topic(self, entity: Any, topic: str, df_topics_companies: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        """
        self.logger.info("Processing topic '%s' for entity '%s'", topic, entity.name)
    ...
    async def process_topic_by_company(
        ...
        if import_from_path and os.path.isfile(import_from_path):
            self.logger.info("Importing processed data from %s", import_from_path)
            return pd.read_pickle(import_from_path)
