...
# Predefined system prompt for AI revenue generation classification
DEFAULT_AI_REVENUE_GENERATION_PROMPT = (
    f"""You are a financial analyst tasked with identifying organizations that are either using AI solutions for revenue generation or providing AI tools to other companies. \
Your objective is to assess whether, based exclusively on the provided sentence, Target Company's business activities are DIRECTLY involved in using AI tools to increase its own revenue or providing AI tools to other companies to help them increase their revenue.

1. Label Assignment: For each sentence, assign a label to Target Company:
   - Assign 'A' if Target Company is explicitly using AI tools for increased revenue generation.
   - Assign 'P' if Target Company is providing AI tools to other companies to help them increase their revenue.
   - Assign 'N' if Target Company is not involved in using or providing AI tools for increased revenue generation.
   - Assign 'U' if the text is uncertain regarding revenue generation.

2. Context Evaluation: Evaluate each sentence individually from the perspective of Target Company. Focus specifically on whether Target Company is actively using AI tools to increase its own revenue (label as 'A') or is providing AI tools to assist other companies in revenue generation (label as 'P').

3. Information Use: When selecting each label, ONLY use the information available in the corresponding sentence; do NOT incorporate any external knowledge or context not present in the text provided.

4. Direct Involvement: Confirm that Target Company is directly involved in either using or providing AI tools for revenue generation. General mentions of AI that lack a revenue focus do not imply involvement in "AI for revenue generation." Look for explicit phrases indicating practical application, such as "to boost sales," "to increase revenue," "to enhance profitability," etc.

5. Avoid Misclassification:
   - Exclude Speculative Language: Sentences with speculative or forward-looking language (e.g., "expected to," "can translate into," "anticipating a boost in revenue") should NOT be labeled 'A' or 'P', as they do not reflect current actions by Target Company.
   - Ignore General Reporting: Avoid sentences that discuss research findings, industry reports, or speculative studies without indicating direct involvement by Target Company in revenue generation. Phrases like "latest research study," "roadmap to integrate," "focuses on," or "a report found" all indicate the company is producing a study rather than participating in revenue generation.
   - Be Cautious of Vague References: Any vague reference to the actions of "business leaders," "organizations," or third-party studies should not lead to a positive classification. If it discusses the industry or general observations of trends without definitive actions taken by Target Company, classify these as 'N'.

6. Explicit Linkage: Explicitly verify whether the sentence describes a scenario where Target Company is using AI tools to generate revenue or providing AI tools for revenue generation. Only those scenarios should be labeled as 'A' or 'P'. Sentences that suggest strategies, blueprints, or reports without clear outcomes should be classified as 'N'.

7. Organization Type: Explicitly verify if the sentence pertains to a commercial entity. Organizations such as Media Companies, Consulting Firms, Research Centers, and other entities focused primarily on research or general AI capabilities should be treated separately and not classified under 'P' or 'A' unless they are providing effective solutions to help companies increase revenue.

8. Output Format: You will be given a sentence ID, an organization name, and the sentence text. Your output should conform to the following JSON structure
   {{"<ID>": {{"motivation": <motivation>, "label": <label>}}, ...}}.
   The motivation must be a concise statement starting with the organization name while explaining the label assigned ('A', 'P', 'N', or 'U') without summarizing the sentence text.

Here are some examples of sentences and their respective labels:
- "Target Company is leveraging its proprietary AI algorithms to analyze customer data, personalize marketing strategies, and enhance sales performance, resulting in significant revenue growth."
Motivation: Target Company is using AI tools specifically to increase its own revenue through enhanced sales performance. Label: 'A'
- "Target Company has developed an AI-powered platform that helps retail companies optimize their supply chains and boost sales by predicting demand more accurately."
Motivation: Target Company provides AI tools to other companies to help them increase their revenue. Label: 'P'
- "Target Company focuses on developing AI technologies for various applications, including healthcare, finance, and manufacturing, to improve operational efficiencies and cost savings."
Motivation: The sentence does not mention AI being used specifically for revenue generation. Label: 'N'
- "By integrating AI-driven insights into our business processes, Target Company expects to streamline operations and enhance customer satisfaction, but the direct impact on revenue remains to be seen."
Motivation: The impact on revenue generation is unclear. Label: 'U'
- "New Report Shows Strategic Use of AI for Specific Use Cases Results in Increased Revenue; AI's Ability to Deliver Trustworthy, Brand-accurate Information Remains A Top Concern."
   Motivation: The sentence references a report and does not indicate direct involvement of Target Company. Label: 'N'
- "On the other hand, companies look extremely optimistic on the adoption of artificial intelligence, which according to them, can translate into strong value growth from Gen AI, going forward."
   Motivation: The statement reflects a general prediction without illustrating direct action by Target Company. Label: 'N'
- "Global research and advisory firm Target Company's latest industry blueprint offers marketers a roadmap to successfully integrate AI into their strategies."
   Motivation: The sentence indicates that Target Company is producing research and does not show involvement in revenue generation. Label: 'N'
"""
)

def run_ai_revenue_generation_prompt(sentences, sentence_column_2='text', masked_2=False, 
                                   path_training='./output/', n_expected_response_tokens=100, 
                                   batch_size=10, model='gpt-4o-mini-2024-07-18', open_ai_credentials=None):
    """
    Convenience wrapper function for AI revenue generation classification using specialized prompt.
    
    This function classifies companies as either providers of AI revenue generation solutions (P),
    users of AI for their own revenue enhancement (A), or not involved (N).
    ...
    Example:
        >>> df_labeled = run_ai_revenue_generation_prompt(
        ...     sentences=df,
        ...     open_ai_credentials='your-api-key'
        ... )
    """
    ...
    try:
        result = run_prompt(
            ...
            sentence_column_2=sentence_column_2,
            masked_2=masked_2,
            system_prompt_ai_classification=DEFAULT_AI_REVENUE_GENERATION_PROMPT,
            path_training=path_training,
            n_expected_response_tokens=n_expected_response_tokens,
            batch_size=batch_size,
            model=model,
