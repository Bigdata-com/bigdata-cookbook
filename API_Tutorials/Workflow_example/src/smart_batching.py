...
class SmartBatchingPlanner:
    ...
    - API Key (legacy): Uses X-API-KEY header
    - Session (Bigdata.com): Uses BigDataSession for session-based auth
    """
    ...
    def __init__(
        ...
        Args:
            api_key: BigData API key (legacy mode). If None, will try to get from environment variable BIGDATA_API_KEY.
        ...
        if session:
            ...
            self.api_base_url = session.api_base_url
            self.api_url = session.comention_endpoint
            self.headers = {}  # Session handles auth
