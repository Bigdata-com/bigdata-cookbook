...
if TYPE_CHECKING:
    from bigdata_session import BigDataSession
...
def get_smart_batching_planner(
    ...
    session: Optional["BigDataSession"] = None
):
    """
    ...
    Args:
        api_key: API key for Bigdata API (legacy mode)
        api_base_url: API base URL (if provided, will set in environment for config)
        session: BigDataSession instance for Bigdata.com authentication (new mode)
...
def plan_search(
    ...
    Args:
        ...
        start_date: Start date in YYYY-MM-DD format
        end_date: End date in YYYY-MM-DD format
        api_key: API key (defaults to BIGDATA_API_KEY env var) - not needed if session is provided
        api_base_url: API base URL (defaults to env var or DEFAULT_API_BASE_URL) - not needed if session is provided
        session: BigDataSession instance for Bigdata.com authentication.
                 If provided, api_key and api_base_url are ignored.
    ...
    if not session:
        ...
        if not api_key:
            raise ValueError("API key or session must be provided. "
                           "Set BIGDATA_API_KEY environment variable or pass a BigDataSession instance.")
    ...
    planner = get_smart_batching_planner(api_key=api_key, api_base_url=api_base_url, session=session)
...
def make_search_request(
    ...
    Args:
        ...
        api_key: API key (legacy mode, not needed if session is provided)
        api_base_url: API base URL (not needed if session is provided)
    ...
    while retry_count < max_retries:
        try:
            with concurrency_limiter:
                ...
                if session:
                    # Use session-based authentication (Bigdata.com)
