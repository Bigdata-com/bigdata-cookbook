...
def generate_interactive_timeline_dashboard_from_unified_df(
    ...
    Uses the original timeline visualization with navigation controls for >7 days.
    ...
    # Get all companies/organizations (not people)
    df_non_people = df[df['Country Code'].notna()].copy()
    
    # Build countries_dict: {"Country": ["Entity1", "Entity2", ...]}
    countries_dict = {}
    ...
    # Group highlights by date and country
    countries_timeline_data = {}
    ...
    for date in timeline_dates:
        ...
        # Get highlights for each country on this date
        for country in available_countries:
    ...
    # Get all companies and organizations
    df_companies = df[
    ...
    for entity in df_companies['Entity'].unique():
        ...
        # Get Novel Daily Summary for each date
        for date in timeline_dates:
            date_str = date.strftime('%Y-%m-%d')
        ...
        for person in available_people:
            ...
            for date in person_dates:
                ...
                if len(day_data) > 0:
                    ...
                    content = f"<strong>Summary:</strong><br>{summary}"
                    if highlights:
                        content += f"<br><br><strong>Highlights:</strong><br>{highlights}"
                    
                    people_summaries_data[person]['date_content'][date_str] = content
    
    # === STEP 5: Generate entity reports data ===
    # Collect all entities with their final summaries
    ...
    # === STEP 7: Generate entity reports HTML ===
    def get_entity_region(entity_name):
        """Get region for entity based on countries_dict"""
    ...
    # Separate companies/orgs from people
    all_companies_from_countries = []
    for company_list in countries_dict.values():
        all_companies_from_countries.extend(company_list)
    all_companies_from_countries = set(all_companies_from_countries)
        ...
        <div class="dashboard-container">
            ...
            <div class="dashboard-content">
                ...
                <div class="content-section">
                    <div class="section-title">⏱️ Interactive Timeline</div>
                    <div class="section-description">
                        Navigate through different perspectives: view highlights by country, explore company narratives, or follow individual people's involvement
                    </div>
                    
                    <div class="timeline-controls">
                        <button class="mode-button active" onclick="switchMode('countries')">🌍 Countries</button>
                        <button class="mode-button" onclick="switchMode('companies')">🏢 Companies</button>
                        <button class="mode-button" onclick="switchMode('people')">👥 People</button>
                    </div>
                    
                    <!-- Entity Selectors -->
                    <div id="countrySelector" class="entity-selector active">
                        <label for="countrySelect">Select Country:</label>
                        <select id="countrySelect" onchange="updateTimeline()">
                            {' '.join([f'<option value="{country}">{country}</option>' for country in available_countries])}
                        </select>
                    </div>
                    
                    <div id="companySelector" class="entity-selector">
                        <label for="companySelect">Select Company:</label>
                        <select id="companySelect" onchange="updateTimeline()">
                            {' '.join([f'<option value="{company}">{company}</option>' for company in available_companies])}
                        </select>
                    </div>
                    
                    <div id="peopleSelector" class="entity-selector">
                        <label for="peopleSelect">Select Person:</label>
                        <select id="peopleSelect" onchange="updateTimeline()">
                            {' '.join([f'<option value="{person}">{person}</option>' for person in available_people])}
                        </select>
                    </div>
                ...
                <div class="content-section">
                    <div class="section-title">📈 Interactive Data Analysis</div>
                    <div class="section-description">
                        Explore the data with interactive charts showing narrative trends and entity analysis
                    </div>
                    {plotly_div}
                </div>
        ...
        <script>
            ...
            function updateTimeline() {{
                ...
                if (currentMode === 'countries') {{
                    ...
                    displayCountriesTimeline(selectedCountry);
                }} else if (currentMode === 'companies') {{
                    const selectedCompany = document.getElementById('companySelect').value;
                    displayCompanyTimeline(selectedCompany);
                }} else if (currentMode === 'people') {{
                    const selectedPerson = document.getElementById('peopleSelect').value;
                    displayPeopleTimeline(selectedPerson);
                }}
            }}
            
            function displayCountriesTimeline(country) {{
                const timelineContent = document.getElementById('timelineContent');
                
                Object.keys(countriesData).sort().forEach((date, index) => {{
                    const highlights = countriesData[date][country] || [];
            ...
            function displayCompanyTimeline(company) {{
                const timelineContent = document.getElementById('timelineContent');
                const companyData = companiesData[company];
                
                if (!companyData) return;
                
                Object.keys(companyData.date_content).sort().forEach((date, index) => {{
                    const content = companyData.date_content[date];
            ...
            function displayPeopleTimeline(person) {{
                const timelineContent = document.getElementById('timelineContent');
                const personData = peopleData[person];
                
                if (!personData) return;
                
                personData.dates.forEach((date, index) => {{
                    const content = personData.date_content[date] || 'No data available for this date';
...
# La nuova funzione verrà aggiunta qui
