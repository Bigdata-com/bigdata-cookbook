...
def bigdata_search(user_request:str) -> str:
    ...
    date_range = AbsoluteDateRange(f"{six_month_ago_str}T08:00:00", f"{today_str}T00:00:00")
    ...
    query_news = Similarity(
        user_request
    ) & Any([Source(source) for source in sources_ids])
...
def bigdata_search_filings_transcripts(user_request:str,company_ticker:str) -> str:
    ...
    # Format as YYYY-MM-DD
    today_str = today.strftime("%Y-%m-%d")
    twelve_month_ago_str = twelve_month_ago.strftime("%Y-%m-%d")
    
    date_range = AbsoluteDateRange(f"{twelve_month_ago_str}T08:00:00", f"{today_str}T00:00:00")
...
@function_tool
def bigdata_chat(user_request:str) -> str:
    """returns up to date information regarding the topic the user requests data for in terms of fundamental analysis"""
    
    chat = bigdata.chat.new("A new chat",formatter=MarkdownLinkFormatter())
    response = chat.ask(user_request, streaming=True, scope=ChatScope.NEWS)
    answer = ""
    for streamingChatInteraction in response:
      answer += streamingChatInteraction
...
def get_fred_series(series_id: str, start_date: str, end_date: str, download_csv: bool = False) -> str:
    ...
    if Fred is None or os.getenv("FRED_API_KEY") is None:
...
def notebook_initialized(bigdata):
    ...
    if not _intialization_sent:
        trace = tracking_services.TraceEvent(event_name = "MultiAgentPortfolioCollaboration", 
